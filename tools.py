...
class AppointmentTools(llm.ToolContext):
    ...
    @llm.function_tool
    async def qualify_and_route_lead(
        self, lead_name: str, lead_phone: str, service_type: str,
        date: str, time: str, lead_status: str = "hot",
    ) -> str:
        """
        Qualify a lead as hot/warm and route via WhatsApp to the assigned broker.
        Call this when a lead is highly interested, has booked, or explicitly wants to schedule a visit.
        Also sends a confirmation WhatsApp to the lead.
        lead_name: lead's full name | lead_phone: with country code
        service_type: what they're interested in | date: YYYY-MM-DD | time: HH:MM
        lead_status: 'hot' (booked/very interested) or 'warm' (interested but uncommitted)
        """
        wa_sid = os.getenv("TWILIO_WA_SID", "") or os.getenv("TWILIO_ACCOUNT_SID", "")
        wa_token = os.getenv("TWILIO_WA_TOKEN", "") or os.getenv("TWILIO_AUTH_TOKEN", "")
        wa_from = os.getenv("TWILIO_WA_FROM", "")
        broker = self.broker_phone
        biz = self.business_name or "our company"
        camp_name = self.campaign_name or "Direct Call"

        if not (wa_sid and wa_token and wa_from):
            return f"Lead qualified as {lead_status}, but WhatsApp not configured. Set TWILIO_WA_SID/TOKEN/FROM in Settings."

        # Build messages with exact formats specified
        lead_msg = (
            f"Hi {lead_name}, thank you for your interest in our premium properties. "
            f"Your visit is scheduled for {date} at {time}. "
            f"Our area expert will contact you shortly to assist you further. "
            f"- {biz}"
        )

        broker_msg = (
            f"\U0001f6a8 HOT LEAD ALERT \U0001f6a8\n\n"
            f"Name: {lead_name}\n"
            f"Phone: {lead_phone}\n"
            f"Interested In: {service_type}\n"
        ...
        try:
            from twilio.rest import Client
            loop = asyncio.get_event_loop()
            client = Client(wa_sid, wa_token)

            # Send to lead
            lead_wa = f"whatsapp:{lead_phone}"
            try:
                await loop.run_in_executor(
                    None, lambda: client.messages.create(
                        body=lead_msg, from_=wa_from, to=lead_wa
                    )
                )
                await log_whatsapp_message(
                    to_number=lead_phone, msg_type="lead",
                    message_preview=lead_msg[:200], status="sent",
                    campaign_id=self.campaign_id, lead_phone=lead_phone,
                )
                results.append(f"\u2705 WhatsApp sent to lead {lead_phone}")
            except Exception as exc:
                await log_whatsapp_message(
                    to_number=lead_phone, msg_type="lead",
                    message_preview=lead_msg[:200], status=f"failed: {exc}",
                    campaign_id=self.campaign_id, lead_phone=lead_phone,
                )
                results.append(f"\u274c Lead WhatsApp failed: {exc}")

            # Send to broker
            if broker:
                broker_wa = f"whatsapp:{broker}"
                try:
                    await loop.run_in_executor(
                        None, lambda: client.messages.create(
                            body=broker_msg, from_=wa_from, to=broker_wa
                        )
                    )
                    await log_whatsapp_message(
                        to_number=broker, msg_type="broker",
                        message_preview=broker_msg[:200], status="sent",
                        campaign_id=self.campaign_id, lead_phone=lead_phone,
                    )
                    results.append(f"\u2705 WhatsApp alert sent to broker {broker}")
                except Exception as exc:
                    await log_whatsapp_message(
                        to_number=broker, msg_type="broker",
                        message_preview=broker_msg[:200], status=f"failed: {exc}",
                        campaign_id=self.campaign_id, lead_phone=lead_phone,
                    )
                    results.append(f"\u274c Broker WhatsApp failed: {exc}")
            else:
                results.append("\u26a0\ufe0f No broker assigned to this campaign")

        except ImportError:
            return "WhatsApp routing failed: Twilio library not installed."
        except Exception as exc:
            return f"WhatsApp routing error: {exc}"

        return f"Lead qualified as [{lead_status.upper()}]. " + " | ".join(results)
