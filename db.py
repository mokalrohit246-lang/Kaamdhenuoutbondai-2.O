...
async def _adb():
    ...
    return await create_client(os.getenv("SUPABASE_URL", ""), os.getenv("SUPABASE_SERVICE_KEY", ""))
...
def init_db() -> None:
    ...
    if not url or not key:
        print("⚠️  SUPABASE_URL or SUPABASE_SERVICE_KEY not set in VPS environment.")
        return
    try:
        db = _sdb()
        ...
        print(f"⚠️  Supabase connection warning: {exc}")
        print("   Run supabase_schema.sql in your Supabase Dashboard → SQL Editor if tables are missing.")
...
async def get_all_settings() -> dict:
    ...
    out: dict = {}
    for k in KNOWN_KEYS:
        # VPS environment variables take priority as the single source of truth:
        ...
        if k in SENSITIVE_KEYS:
            out[k] = {"value": "", "configured": is_configured}
        else:
            out[k] = {"value": effective_val, "configured": is_configured}
    return out
...
async def get_setting(key: str, default: str = "") -> str:
    # 1. VPS environment variable is the single source of truth
    env_val = os.getenv(key, "").strip()
    if env_val:
        return env_val
    # 2. Database setting fallback
    try:
        db = await _adb()
...
async def log_call(
    ...
    # Auto-increment campaign consumed_minutes if campaign_id is set
...
async def increment_consumed_minutes(campaign_id: str, minutes: int) -> None:
    """Add minutes to a campaign's consumed count. Auto-pause if over budget."""
    db = await _adb()
...
async def check_campaign_budget(campaign_id: str) -> bool:
    """Returns True if campaign still has budget (or has no cap). False if over."""
    campaign = await get_campaign(campaign_id)
    if not campaign:
        return False
    allocated = campaign.get("allocated_minutes") or 0
    if allocated == 0:
        return True  # No cap = unlimited
...
async def add_campaign_minutes(campaign_id: str, extra_minutes: int) -> bool:
    """Top up allocated minutes for a campaign."""
    db = await _adb()
    campaign = await get_campaign(campaign_id)
    if not campaign:
        return False
    ...
    # If was paused due to budget, reactivate
